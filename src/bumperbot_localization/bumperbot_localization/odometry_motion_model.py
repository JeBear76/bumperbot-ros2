...
from math import sin, cos, atan2, fabs, sqrt, pi
import random
import time
...
class OdometryMotionModel(Node):
    def __init__(self):
        ...
        if self.nr_samples >= 0:
            ...
            self.get_logger().fatal(f"Invalid Number of Samples: {self.nr_samples}")
            return
    ...
    def odomCallback(self, msg):
        ...
        odom_y_increment = msg.pose.pose.position.y - self.last_odom_y
        ...
        transl_variance = self.alpha3 * delta_transl + self.alpha4 * (
